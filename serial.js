...
class UsbEndpointUnderlyingSource {
    ...
    pull(controller) {
        ...
        this.device_.transferIn(this.endpoint_.endpointNumber, chunkSize).then((result) => {
...
class SerialPort {
    ...
    constructor(device, polyfillOptions) {
        ...
        this.inEndpoint_ = this.transferInterface_.alternates[0].endpoints.find(x => x.direction === 'in'); // findEndpoint(this.transferInterface_, 'in');
        this.outEndpoint_ = this.transferInterface_.alternates[0].endpoints.find(x => x.direction === 'out'); // findEndpoint(this.transferInterface_, 'out');
    }
    /**
    ...
    async setLineCoding() {
        ...
        if (result.status === 'stall') {
            let exeptText = 'Failed to set line coding.';
            try {
                await this.device_.clearHalt('in', this.inEndpoint_.endpointNumber);
                await this.device_.clearHalt('out', this.outEndpoint_.endpointNumber);
            } catch (e) {
                exeptText += ' ' + e.toString();
            }

            //throw new DOMException('NetworkError', exeptText);
